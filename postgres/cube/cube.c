...
cube_send(PG_FUNCTION_ARGS)
{
	...
	// if (cube) O-O-O-K;
	pq_sendint(&buf, cube->vl_len_, 4);
	pq_sendint(&buf, cube->dim, 4);
...
cube_recv(PG_FUNCTION_ARGS)
{
	...
	errmsg("unimplemented recv in cube type")));  

	NDBOX		*result = NULL;	
/*
	StringInfo	buf = (StringInfo) PG_GETARG_POINTER(0);

	result = (NDBOX *) palloc(sizeof(NDBOX));

	result->var_len_ = pq_getmsgint(buf);
	result->dim = pq_getmsgint(buf);
*/
	PG_RETURN_NDBOX	(result);
}
